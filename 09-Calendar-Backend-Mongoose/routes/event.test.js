...
afterAll( ()=>{
  // close the db connection so our test can
  // end gracefully in the end
  const { db } = require('../db');
  db.close();
});
...
test('simple delete event', async ()=>{
  const createRes = await create({ title: 'dentist' });
  const deleteRes = await remove(createRes.data._id);
});
...
test('simple patch event', async ()=>{
  const createRes = await create({ title: 'dentist' });
  expect(createRes.data.title).toMatch('dentist');
  const patchRes = await patch(createRes.data._id,{ title: 'barber' });
